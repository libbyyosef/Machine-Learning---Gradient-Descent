...
class LogisticRegression(BaseEstimator):
        ...
        if self.penalty_ == "none":
            ...
            logistic_module = LogisticModule(
                np.random.normal(0, 1, X.shape[1]) / np.sqrt(
                    X.shape[1]))
            reg_module=RegularizedModule(logistic_module,reg,self.lam_,np.random.normal(0, 1, X.shape[1]) / np.sqrt(X.shape[1]),
    ...
    def _predict(self, X: np.ndarray) -> np.ndarray:
        ...
        responses = (proba >= self.alpha_).astype(int)
        return responses
    ...
    def predict_proba(self, X: np.ndarray) -> np.ndarray:
        ...
        """
        X = np.insert(X, 0, 1, axis=1) if self.include_intercept_ else X
        X_coefs_product = X @ self.coefs_
        exp_X_coefs = np.exp(-X_coefs_product)
        res = 1 / (exp_X_coefs + 1)
        return res
